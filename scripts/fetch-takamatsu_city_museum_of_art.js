...
function splitHtmlBySpecialSection(html) {
  ...
  if (specialStart === -1) {
    ...
    const sectionEndCandidates = ["コレクション展", "その他展覧会", "日本伝統漆芸展", "企画展", "常設展"];
    const endPositions = sectionEndCandidates
      .map((label) => cleaned.indexOf(label, fallbackStart + SPECIAL_SECTION_LABEL.length))
      .filter((pos) => pos !== -1);
    const fallbackEnd = endPositions.length > 0 ? Math.min(...endPositions) : cleaned.length;
...
function extractSpecialEventsFromSection(sectionHtml) {
  ...
  // 特別展はアンカーの直後に日付が出るため、アンカーのHTML位置から周辺を切り出して日付を探す。
  const dateIndicator = /\d{4}\s*年\s*\d{1,2}\s*月\s*\d{1,2}\s*日/;
  const events = [];
  const seen = new Set();
  ...
  const previewPairs = [];
  const lookaheadChars = 900;
  const anchorMatches = [...sectionHtml.matchAll(/<a[^>]*href=["']([^"']+)["'][^>]*>([\s\S]*?)<\/a>/gi)];
  ...
  for (const anchor of anchorMatches) {
    ...
    let dateLine = "";
    const sliceStart = anchor.index ?? 0;
    const fragment = sectionHtml.slice(sliceStart, sliceStart + lookaheadChars);
    const fragmentLines = extractLinesFromHtml(fragment);
    for (const line of fragmentLines) {
      if (dateIndicator.test(line)) {
        dateLine = line;
        break;
      }
    }
